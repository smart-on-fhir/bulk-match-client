...
describe.skip("Logging", function () {
  ...
  describe("kickoff", () => {
    ...
    it("includes request parameters in kickoff log entries", async () => {
      ...
      expect(entry.eventDetail.exportUrl).to.include(
        mockServer.baseUrl + "/Patient/$bulk-match",
      );
  ...
  describe("status events", () => {
    it("logs status_progress events in case of 202 status responses", async () => {
      ...
      const logs = log
        ...
        .filter(Boolean)
        .map((line) => JSON.parse(line))
        .filter((l) => l.eventId === "status_progress");

      expect(logs.length, "must have 3 status_progress log entries").to.equal(
        3,
      );
  ...
  describe("download events", () => {
    it("download without errors", async () => {
      ...
      // /downloads/docRef -----------------------------------------------
      {
        ...
        expect(entries[0].eventDetail.itemType).to.equal("output");
        expect(entries[0].eventDetail.resourceType).to.equal(
          "DocumentReference",
        );
      }
      ...
      // /document.pdf ---------------------------------------------------
      {
        const entries = logs.filter(
          (e) =>
            e.eventId === "download_request" &&
            e.eventDetail.fileUrl === mockServer.baseUrl + "/document.pdf",
        );
        expect(
          entries.length,
        ...
        expect(entries[0].eventDetail.fileUrl, "invalid fileUrl").to.equal(
          mockServer.baseUrl + "/document.pdf",
        );
      ...
      // export_complete -------------------------------------------------
      {
        ...
        expect(
          entries[0].eventDetail.resources,
          "must report 5 resources",
        ).to.equal(5);
    ...
    it("logs download_error events on invalid resource count", async () => {
      ...
      expect(entry.eventDetail.body).to.equal(null);
      expect(entry.eventDetail.message).to.equal(
        "Expected 30 resources but found 2",
      );
    });
