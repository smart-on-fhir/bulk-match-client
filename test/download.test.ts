...
describe('download', function() {
    ...
    it("downloadFile method works", async () => {
        ...
        // @ts-ignore
        const client = new BulkMatchClient({ ...baseSettings, fhirUrl: mockServer.baseUrl, destination: join(__dirname, "./tmp/downloads") })
    ...
    it ("Integration: Manifest will be parsed and all output files will be downloaded", async () => {
        ...
        expect(readFileSync(join(__dirname, './tmp/downloads/file_1.ndjson'), "utf8")).to.equal(JSON.stringify(mockResponse))
    })
