  ...
  describe("BulkMatchClient", function () {
    ...
    describe("_parseResourceNdjson", function () {
      it("Should work on NDJSON ", () => {
        ...
        const interiorJson = ndjsonExample
          .split("\n")
          .map((j) => JSON.parse(j));
        // @ts-expect-error
        expect(client._parseResourceNdjson(ndjsonExample)).to.equal(
          interiorJson,
        );
      });
    ...
    describe("_parseResourceStringOption", function () {
      ...
      it("Should work on absolute path to valid JSON objects", async () => {
        ...
        const resourceFixture = JSON.parse(
          readFileSync(resourcePathAbsolute, "utf-8"),
        );
        const resource =
          // @ts-expect-error
      ...
      it("Should work on relatives path, relative-to where the command is invoked", async () => {
        ...
        const resourcePathRelative = path.resolve(
          "./test/fixtures/patient-obj.json",
        );
        const resourceFixture = JSON.parse(
          readFileSync(resourcePathRelative, "utf-8"),
        );
        const resource =
          // @ts-expect-error
      ...
      it("Should work on an NDJSON file", async () => {
        ...
        // @ts-expect-error
        expect(await client._parseResourceStringOption(ndjsonPath)).to.equal(
          finalJson,
        );
      });
    ...
    describe("_parseResourceOption", function () {
      it("Should work on valid strings, wrapping the return value in an array", async () => {
        ...
        const resourcePathFixture = JSON.parse(
          readFileSync(resourcePathRelative, "utf-8"),
        );
        const resourceObj =
          // @ts-expect-error
